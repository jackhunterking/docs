...
AdsGateway cannot see Pixel network calls directly (Pixel goes browser -> Meta), so for parity debugging we recommend embedding a small `browser_pixel` object in the **same CAPI request** you send to AdsGateway.

Only do this in debug/testing mode (for example when `?ag_debug=1` or a `test_event_code` is present).
...
- `Pixel: sent/blocked/unknown` derived from `browser_pixel`
